...
A Portal into the Nbc and Substrate networks. Provides a view and interaction layer from a browser.

This can be accessed as a hosted application via https://nbc.js.org/apps/ or you can access the IPFS hosted version via https://nbc.js.org/apps/ipfs (via hash) or https://dotapps.io (via ipns) to explore any of the supported Nbc and Substrate chains.
...
To start off, this repo (along with others in the [@nbc](https://github.com/nbc-js/) family) uses yarn workspaces to organize the code. As such, after cloning dependencies _should_ be installed via `yarn`, not via npm, the latter will result in broken dependencies.

To get started -

1. Clone the repo locally, via `git clone https://github.com/nbc-js/apps <optional local path>`
...
5. Ready! Now you can launch the UI (assuming you have a local Nbc Node running), via `yarn run start`
...
```
docker run --rm -it --name nbc-ui -e WS_URL=ws://someip:9944 -p 80:80 jacogr/nbc-js-apps:latest
```

To build a docker container containing local changes -

```
docker build -t jacogr/nbc-js-apps -f docker/Dockerfile .
```
...
IPFS allows sharing files in a decentralized manner in a similar fashion the nbc network exchanges blocks. IPFS works best when many nodes seed the same data. Nodes can seed specific data by **pinning** them.

你可以 pin with the following command:

```
curl -s https://nbc.js.org/apps/ipfs/pin.json | jq -jr .IpfsHash | xargs -0 -I CID ipfs pin add --progress CID
```

Here is a script you can save as `/usr/local/bin/nbcjs-ipfs-pin.sh`:
...
```
$ /usr/local/bin/nbcjs-ipfs-pin.sh
...
SHELL=/bin/bash
HOME=/
0 * * * * /usr/local/bin/nbcjs-ipfs-pin.sh >/dev/null 2>&1
```
...
Qmd8UnRQiBobm4qb6dhiC1HoQ7SvwZrWJenoN3JPEV3iiF 480594  nbcjs.3af757ad.js
...
* Mac: `~/Library/Application Support/nbc-apps/nbc-accounts`
* Linux: `~/.config/nbc-apps/nbc-accounts` (or `$XDG_CONFIG_HOME/nbc-apps/nbc-accounts` if `$XDG_CONFIG_HOME` is defined)
* Windows: `%APPDATA%\nbc-apps\nbc-accounts`

For more details on the desktop app, head over to [Electron package README](https://github.com/nbc-js/apps/blob/master/packages/apps-electron/README.md).
